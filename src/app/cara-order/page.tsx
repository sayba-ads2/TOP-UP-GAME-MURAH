...
export const metadata: Metadata = {
  title: 'Cara Order Top Up Game — Panduan Lengkap untuk Pemula',
  description:
    'Panduan langkah demi langkah cara top up game di Top Up Game Murah: dari memilih game, menemukan User ID, memilih nominal, sampai membayar lewat QRIS atau transfer bank.',
  ...
  return (
    <>
      <JsonLd
        data={[
          ...
          ]),
          {
            ...
            '@type': 'HowTo',
            name: 'Cara Top Up Game di Top Up Game Murah',
            description:
      ...
      <div className="mx-auto max-w-3xl px-4 py-12">
        <h1 className="text-2xl font-extrabold text-ink-100">Cara Order Top Up Game</h1>
        ...
        <ol className="mt-8 space-y-3">
          {STEPS.map((step, index) => (
            <li key={step.title} className="card-surface flex gap-4 p-5">
              <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-flame-500 text-sm font-bold text-white">
                {index + 1}
              </span>
              <div>
                <h2 className="text-sm font-bold text-ink-100">{step.title}</h2>
        ...
        <div className="card-surface mt-8 p-5">
          <h2 className="text-sm font-bold text-ink-100">Masih bingung?</h2>
          <p className="mt-2 text-sm leading-relaxed text-ink-400">
